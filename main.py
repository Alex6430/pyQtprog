...
mas = search()
cur = cnxn.cursor()
strk = cur.execute('select top(1) ID_K from MatrixSize order by ID_KM desc')
sk = strk.fetchone()[0]
strm = cur.execute('select top(1) ID_M from MatrixSize order by ID_KM desc')
sm = strm.fetchone()[0]
K = int(sk)
M = int(sm)
cur.close()
...
def printImage():
    ...
    # pylab.show()
    cur = cnxn.cursor()
    # K = cur.execute("select top(1) id_graph from Graph order by id_graph desc")
    id_graph = cur.execute("select top(1) id_graph from Graph order by id_graph desc")
    id = id_graph.fetchone()[0] + 1
    strnew = 'graf' + str(id) + '.png'
    pylab.savefig(strnew)

    cur.execute("insert into Graph(images_graph) values (?)", strnew)
    cur.commit()
    cur.close()
    print("main print")
    return id
